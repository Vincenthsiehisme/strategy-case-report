...
v2.6 起採用分段執行設計：分析 context（Phase 1–6）與輸出 context（Phase 6.5–7）分離，各自乾淨，避免長 context 造成的注意力衰減。
...
**截點（人類判斷）：**
Phase 6 批判完成後，AI 輸出截點 JSON（`report_data.json`）。使用者確認內容後，手動開啟新對話貼入，啟動第二段輸出 context。
...
│   ├── phases-4-7.md                 # Phase 3.5–7 完整執行規格（含截點定義）
│   ├── plain-text-format.md          # 純文字分析稿格式模板（路徑 D）
│   ├── report-data-schema.md         # 截點 JSON 資料結構定義（三欄結構）
│   ├── root-search.md                # Phase 3.5 扎根搜尋規格
│   ├── visual-ref.md                 # 視覺系統決策規格與搜圖規則
...
**兩段 context 截點**
Phase 6 結束後輸出截點 JSON，人類確認後開啟新對話啟動第二段。第二段只讀 JSON 和版型規格，context 大幅降低，版型執行品質更穩定。
