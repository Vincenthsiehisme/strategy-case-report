...
1. 以單一 HTML artifact 輸出完整 deck，CSS 變數統一管理色彩與字體，Google Fonts CDN 引入字體
2. 依 Phase 4 藍圖的頁面結構逐頁製作，每頁 `<section>` 套用對應版型節奏
3. 模型圖表以 HTML/CSS 製作，不用圖片替代
4. 論證型配圖依 C 模式執行：先搜圖 → 通過證據檢查才嵌入 → 搜不到或不符標準輸出配圖需求描述
5. 輸出來源附錄與 Image Source Appendix（作為報告末頁 section）
