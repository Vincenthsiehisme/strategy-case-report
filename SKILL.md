...
| 輸出形式 | HTML artifact deck（預設）/ 純文字分析稿（Sprint Mode 可選） |
...
Visual Brief 確認後，進入 HTML artifact 製作。

**HTML 結構規範：**
採單頁滾動結構（scroll-based）。每頁為獨立 `<section>`，`min-height: 100vh`。頂部固定錨點導航列，讓使用者可以跳頁。不採 JS 換頁動畫——避免單一 artifact 複雜度過高、在不同環境下失效。

**效能控制規則（完整報告約 25–27 個 section，需主動管理）：**
- CSS 動畫只套用在前 5 個 section，其餘頁面靜態呈現，避免渲染過載
- 嵌入圖片統一限制寬度：`max-width: 600px`，避免大圖拖慢載入
- 若圖片超過 3 張，改用 `loading="lazy"` 屬性延遲載入
- 若 artifact 渲染明顯緩慢，主動告知使用者可拆成兩份 artifact（案例 1–2 / 案例 3 + 附錄）

**製作前確認清單：**

Full Mode：
- [ ] Visual Brief 已由使用者確認
- [ ] 色彩系統（5 個 CSS 變數）已鎖定
- [ ] 字體配對（Display + Body）已鎖定
- [ ] 五種版型節奏的具體呈現方式已定義

Sprint Mode（HTML artifact）：
- [ ] 精簡版 Visual Brief 已確認（色彩 + 字體兩個維度）
- [ ] 色彩系統（5 個 CSS 變數）已鎖定
- [ ] 字體配對（Display + Body）已鎖定

Sprint Mode（純文字稿）：
- [ ] 無需確認視覺系統，直接輸出純文字格式（見 `references/plain-text-format.md`）

**輸出步驟（HTML artifact 路徑）：**
1. Visual Brief 確認後，將分析內容與視覺系統填入 `report_data.json`（格式見 `scripts/report_schema.json`）
2. 呼叫 build script：
   ```
   python scripts/build_report.py --input report_data.json --output report.html
   ```
3. 若 script 回報 validation error，依錯誤訊息修正 JSON 後重跑
4. 論證型配圖依 C 模式執行：先搜圖 → 通過證據檢查才嵌入（填入 JSON 的 `image_status: found`）→ 搜不到則填 `image_status: needed` 並在 `image_brief` 說明需求
5. 輸出的 `report.html` 即為完整 deck，含來源附錄與 Image Source Appendix

**輸出步驟（純文字稿路徑）：**
依 `references/plain-text-format.md` 的格式模板逐案輸出，不製作 HTML，不配圖，完成後附來源清單。
...
使用者在 Intake Checklist 選擇純文字輸出時，Phase 7 改為輸出純文字格式，不製作 HTML artifact，不需要 Visual Brief（跳過 Phase 6.5）。

完整格式模板與注意事項：→ 詳見 `references/plain-text-format.md`
...
### Full Mode（8 項）
Full Mode 預設輸出 HTML artifact deck，不支援純文字稿輸出。
