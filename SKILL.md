...
| 環境 | 判斷方式 | 執行路徑 |
|------|---------|---------|
| Claude.ai（網頁 / App） | 無法執行 bash / python 指令 | 直接生成 HTML artifact |
| Claude Code / Cowork | 有 bash 工具存取 | 填入 JSON → 呼叫 build script |

---

**路徑 A｜Claude.ai 直接生成 HTML artifact**

1. 依 Visual Brief 鎖定的 CSS 變數與字體，以單一 HTML artifact 輸出完整 deck
...
**路徑 B｜Claude Code / Cowork 呼叫 build script**

1. 將分析內容與視覺系統填入 `report_data.json`
   格式規範見 `scripts/report_schema.json`
2. 執行：
   ```bash
