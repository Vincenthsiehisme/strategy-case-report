...
本文件是 `strategy-case-report` SKILL v2.3 的配套規格文件。
...
| v1.1 | 2026-03-20 | 對齊 SKILL v2.2：統一字體規格表格欄位、釐清版型庫定義（Dark Full）、新增文件定位與版本說明、整合 Gotchas 為唯一來源、新增版型 HTML 骨架 |
| v1.2 | 2026-03-20 | 中文優先重寫：第 2 節字體規格加入中文硬規則與字重補償邏輯、移除 6.5px 下限、Source 改色彩+字重標注；第 3.2 節改為密度單位制（硬上限 4 單位）；第 3.3 節版型庫重構為資訊形狀分類（Content-A/B/C/D/E/F、DarkFull-Recap/Insight 獨立）；第 6 節補三條中文寫作硬規則；第 7.2 節檢查清單對齊新規格 |
...
| Title | 每頁標題（`sh-title`） | 16–20px | 800 | 單行為佳，最多 2 行；中文最多 16 字，超過拆副標 |
| Subtitle | Insight box / Quote box 核心主張句 | 11–13px | 700 | 最多 3 行；中文最多 40–44 字；這裡放主張，不放摘要 |
| Body | Card / steal card 內文 | 10.5–11.5px | 500 | 見 2.2 節硬規則 |
| Label | Eyebrow、tag、小標籤 | 8–9px | 700 | 全大寫 + `letter-spacing: 1.5–3px` |
| Source | 來源標注、初步判讀標注 | 8px | 500 | 色彩減淡（見 2.2 節），**不使用 italic** |
| Metric | 關鍵數字 | 18–26px | 800 | 必須與 Label 層級搭配，形成視覺錨點 |

### 2.2 中文 Slide 字體硬規則

以下規則優先於上方層級表的範圍值。遇到衝突，以硬規則為準。

**字體大小下限：**
- 中文 Slide body 不得低於 **10.5px**
- 深色背景頁（Cover / Section Divider / DarkFull-Recap / DarkFull-Insight）的 body 不得低於 **11px**
- Source 層級不得低於 **8px**
- **無任何例外的最小值**：8px。低於此值在投影場景中文字不可辨。

**當 body 需要壓到 10.5px 以下，唯一解法是拆頁，不是縮字。**

**卡片密度上限（同時適用）：**
- 一張 card 最多 **3 個語意段**（說明 / 第一步 / 前置條件 / 邊界條件各算一段）
- 一張 card 中文字數超過 **40 字**，視為過密，必須拆卡或移至下一頁
- 若頁面下半部留白超過 25% 但 body 已低於 11px，視為**版型失敗**，不是設計留白——解法是換版型或拆頁，不是縮字
...
**字重補償原則：** system font stack 在不同執行環境的中文渲染結果不一致，實際視覺往往比預期細、灰、小。補償方式是將所有字重上調一級：

| 傳統英文 spec | 中文 Slide 補償後 |
|-------------|----------------|
| font-weight: 400 | → **500** |
| font-weight: 600 | → **700** |
| font-weight: 700 | → **800** |

Body 層級使用 500，Label / Source 使用 700，不使用 400。這不是加粗風格，是補償中文字型在 PDF 渲染中的視覺損耗。

**Source 層級標注方式：** 不使用 `font-style: italic`（對中文視覺效果極弱）。改用色彩減淡 + 字重對比區分：

```css
/* Source 標注 */
font-size: 8px;
font-weight: 500;
color: rgba(255,255,255,0.45);  /* 深色背景頁 */
/* 或 */
color: #94A3B8;                  /* 白色背景頁 */
```
...
| 支撐元素 | 說明 | 密度單位 |
|---------|------|---------|
| Metric strip | 大數字 + 短說明，通常 3–4 個並排 | 1 單位 |
| Short card | 有標題的資訊方塊，內文 ≤ 1 個語意段 | 1 單位 |
| Card | 有標題的資訊方塊，內文 2–3 個語意段 | 1 單位 |
| Steal card | 方法卡，含說明 + 第一步 + 前置/邊界條件 | 2 單位 |
| Flow | 流程步驟圖，通常 4 個步驟橫排 | 2 單位 |
| Quote box | 核心主張句，深色背景，字重 700 | 1 單位 |
| Reframe pair | 舊說法 vs. 新說法並排 | 2 單位 |
| Recap matrix | 跨案比較表格 | 4 單位 |

**硬規則：每頁支撐元素密度總和不得超過 4 單位。** 超過 4 單位必須拆頁，不是縮字、不是壓縮間距。

**附加限制：**
- 含 Source note 的頁面，主體 card 數量自動減 1（等同增加 0.5 單位負擔）
- 同一頁 Source note 超過 2 行，不得使用三欄版型
- 含 steal card（2 單位）的頁面，每頁最多 2 張 steal card（已達上限 4 單位）

**每頁禁止出現的格式：**
- 連續段落文字超過 3 行未換 card
- 多層巢狀 bullet point
- 超過 5 個 card 在同一頁（無論單位計算結果）

### 3.3 版型庫

每頁必須選用以下版型之一。版型以**資訊形狀**定義，不只以頁面語義定義。不允許自行發明未定義的版型。

**選版型的判斷順序：先看頁面的主體資訊形狀，再看語義。**
...
#### Content-A｜Hero + 2 Evidence
- 資訊形狀：1 個主張區（大字或 Quote box）+ 最多 2 個證據卡
- 適用頁面：Real problem、Tension
- 規則：不允許在主張區外加第 3 個大卡；證據卡每張最多 2 個語意段
- 密度上限：Quote box（1）+ 2 × Card（1+1）= 3 單位 ✓

---

#### Content-B｜Metric Strip + Short Cards
- 資訊形狀：1 條 metric strip（3–4 個數字並排）+ 2–3 張短卡
- 適用頁面：Case in one line
- 規則：短卡只放單一語意段（說明或結論），不放前置條件 / 邊界條件
- 密度上限：Metric strip（1）+ 3 × Short card（1+1+1）= 4 單位 ✓

---

#### Content-C｜Reframe Pair
- 資訊形狀：舊框架 vs. 新框架並排 + 1 個放棄項說明
- 適用頁面：Strategic reframe、Tension（Reframe 合頁時）
- 規則：每欄只允許 1 句核心句 + 最多 1 行說明；不允許在雙欄外加第三個資訊區塊
- 密度上限：Reframe pair（2）+ Short card（1）= 3 單位 ✓

---

#### Content-D｜Flow 4 Steps
- 資訊形狀：4 個步驟橫排（或 2×2 排列）
- 適用頁面：The system
- 規則：每個 step 只允許標題 + 最多 2 行說明；不允許在 Flow 外加超過 1 個 card
- 密度上限：Flow（2）+ Card（1）= 3 單位 ✓

---

#### Content-E｜Decision Cards 2-up
- 資訊形狀：每頁最多 2 張方法卡（steal card）
- 適用頁面：What to steal、Conclusion 行動框架
- 規則：每卡允許包含「說明 + 具體第一步 + 前置條件 / 邊界條件」多段內容；正因內容密，每頁硬限 2 張
- 密度上限：2 × Steal card（2+2）= 4 單位 ✓（已滿，不得再加任何元素）

---

#### Content-F｜Proof Split
- 資訊形狀：左欄事實區 + 右欄判讀區，搭配 metrics
- 適用頁面：Why it lands
- 結構：左欄放已驗證事實（metric strip 或條列）；右欄放初步判讀段落；頂部或底部放 1–2 個 metric 錨點
- 規則：事實欄與判讀欄必須有視覺分隔（色條或背景色差）；判讀欄文字必須有「初步判讀」標注；不允許左右欄使用相同視覺處理
...
- 背景：深色（主色）
- 用途：Cross-Case Recap，呈現三案橫向比較表
- 特徵：頂部金色或強調色細條；表格在深色背景上，文字用白色；重點欄位用強調色標注；**無大型數字底紋**（與 Section Divider 的識別差異）
- 與 DarkFull-Insight 的差異：Recap 是資料密集的比較表；Insight 是單一大主張句
...
- 特徵：1 個大主張句佔版面主體，字重 800，留白為設計語言的一部分；可加 1–2 行補充說明；**無表格、無多卡**
- 密度上限：1 個 Quote box（1 單位），是此版型的唯一主體
...
- **頁數說明：來源附錄通常 2 頁起跳，不要硬壓成 1 頁。** 寧可分兩頁，Source 字體保持 8px，也不要縮到不可讀。

---

**三欄版型限制：** 三等寬直欄只可用於短卡（每欄 ≤ 1 個語意段）。凡含 steal card 或多語意段內容，不允許使用三欄版型。
...
| 框架說明 | Content-A | 1 |
| 案例總覽 | Content-B | 1 |
| Section Divider（每案） | Section Divider | 1 × 案例數 |
| Cross-Case Recap | DarkFull-Recap | 1 |
| 結論 Phase 框架 | Content-E | 1 |
| Core Insight | DarkFull-Insight | 1 |
| 來源附錄 | Appendix-2col | **2 頁起跳**，不硬壓 1 頁 |
...
以下三條僅適用於中文內容，優先於其他寫作建議。

**規則一｜Title 字數上限**  
中文 `sh-title`（頁面標題）最多 **16 字**。超過 16 字，拆成主標 + 副標兩行，不壓縮字體。

**規則二｜Quote box 字數上限**  
中文 Quote box / Insight box 的主張句最多 **40–44 字**。超過 44 字代表這個主張需要再收斂，不是加長字數。

**規則三｜Source 標注不使用 italic**  
`font-style: italic` 對中文字型視覺效果極弱，無法有效區分標注層級。改用色彩減淡 + 字重對比作為區分方式（見第 2.3 節 Source 標注規格）。
...
- [ ] 中文 body 字體 ≥ 10.5px（深色背景頁 ≥ 11px）
- [ ] Source 層級字體 ≥ 8px
- [ ] Source 層級無 `font-style: italic`，改用色彩減淡 + font-weight: 500
- [ ] 所有 body / card 內文字重 ≥ 500（不使用 400）

**版型完整性：**
- [ ] 封面頁（Cover）存在
- [ ] 每個案例有 Section Divider
- [ ] 每個 Content-A/B/C/D/E/F 頁有頂部 4px 色條
- [ ] Cross-Case Recap 頁使用 DarkFull-Recap 版型
- [ ] Core Insight 頁使用 DarkFull-Insight 版型
- [ ] 每頁版型為 3.3 節定義的版型之一（無自創版型）
- [ ] 每頁密度單位總和 ≤ 4 單位
...
**錯誤 1｜中文 body 字體太小**  
用了 8.5–9.5px 的 body 字體，在投影場景或深色背景頁中文完全不可辨。  
→ **修正**：中文 body ≥ 10.5px，深色背景頁 ≥ 11px。Source ≥ 8px。任何頁面需要壓字才能塞進去，唯一解法是拆頁。

**錯誤 2｜字重設為 400，中文渲染偏細**  
system font stack 在不同環境渲染結果不一，400 字重在 PDF 中往往顯示為比預期更細、更灰。  
→ **修正**：body 使用 font-weight: 500，label/source 使用 700。不使用 400。

**錯誤 3｜Source 標注用 italic 區分**  
`font-style: italic` 對中文字型視覺效果極弱，無法有效與正文區分。  
→ **修正**：Source 改用色彩減淡（rgba 白色 0.45 或 #94A3B8）+ font-weight: 500 區分，不使用 italic。

**錯誤 4｜頁面下半留白大但字體已壓小**  
字體已低於 11px，頁面下半卻還有 30% 留白。這不是設計留白，是版型失敗。  
→ **修正**：換版型（改用更大密度上限的版型）或拆頁，讓字體回到合規範圍。

**錯誤 5｜用 Content 通用版型，塞了過多元素**  
舊的 Content 版型太泛，導致 quote + 3 cards + flow + source 全部擠在同一頁。  
→ **修正**：依資訊形狀選 Content-A/B/C/D/E/F 子版型。每頁計算密度單位，超過 4 單位必須拆頁。

**錯誤 6｜一頁放了 3 張 steal card**  
每張 steal card = 2 單位，3 張 = 6 單位，超出硬上限 4 單位。  
→ **修正**：Content-E 版型硬限每頁最多 2 張 steal card。超過就拆成兩頁。

**錯誤 7｜Section Divider 用白色背景**  
Section Divider 的功能是「視覺轉折 + 案例預告」，白色背景無法製造節奏切換感。  
→ **修正**：Section Divider 永遠使用深色（主色）背景，沒有例外。

**錯誤 8｜忘記頂部色條**  
Content-A/B/C/D/E/F 頁缺少頂部 4px 色條，讀者無法快速判斷在看哪個案例。  
→ **修正**：每個 Content 子版型的頂部第一個元素必須是 `height:4px` 的色條，顏色與該案例識別色一致。

**錯誤 9｜使用 CSS variable**  
`var(--primary)` 在 WeasyPrint 中不被支援，導致顏色全部失效（通常變成黑色或透明）。  
→ **修正**：所有顏色改為硬編碼 hex，轉換前執行第 7.2 節檢查清單。

**錯誤 10｜DarkFull-Recap 與 Section Divider 混用**  
兩者都是深色背景，但用途不同：Section Divider 是案例起點（有大型數字底紋），DarkFull-Recap 是全報告收尾轉折（無數字底紋，是資料密集的比較表）。  
→ **修正**：Recap 頁用 DarkFull-Recap，Core Insight 頁用 DarkFull-Insight，Section Divider 只用於案例第一頁。三者不可互換。

**錯誤 11｜`overflow: hidden` 被移除**  
當內容過多時，試圖移除 `overflow: hidden` 讓內容溢出。這會導致內容跨頁，破壞投影片邊界。  
→ **修正**：`overflow: hidden` 不可移除。內容過多的解法只有一個：拆成兩頁。

**錯誤 12｜整份報告沒有深色背景頁**  
所有頁面都是白色背景，視覺無節奏感。  
→ **修正**：Cover、每案 Section Divider、DarkFull-Recap、DarkFull-Insight 必須是深色背景頁。

**錯誤 13｜Quote box 放的是摘要而非主張**  
Quote box 放了「這個案例說明了數位轉型的重要性」這種無意義的摘要句。  
→ **修正**：Quote box 的文字必須是完整的策略主張，可以引發爭議或讓人思考。參照第 6.1 節。中文不得超過 44 字。
