...
version: "2.1"
updated: "2026-03-21"
paired-with: "strategy-case-report SKILL v2.4"
...
| v2.1 | 2026-03-21 | 補元素字數上限表（3.2）、Steal card 寫作原則（7.4）、DarkFull-Insight 行動指向要求；配對 SKILL v2.4 |
...
| 元素 | 上限 | 超過時 |
|------|------|--------|
| 頁面標題（sh-title） | 16 字 | 拆副標 |
| Quote box 主張句 | 44 字 | 拆兩句或精簡 |
| Card 標題 | 16 字 | 必須縮短 |
| Card 內文（每段） | 40 字 | 拆卡 |
| Steal card 中欄標題 | 24 字 | 必須縮短 |
| Steal card 中欄說明文（全欄合計） | 70 字 | 拆頁或移附錄 |
| Steal card 右欄（具體第一步 + 邊界條件合計） | 55 字 | 精簡至核心動作 |
| Section Divider 說明文 | 50 字 | 必須精簡 |
| 結尾頁行動指向句 | 30 字，1 句 | 只保留最核心的一個問題 |

字數超過上限的唯一解法是**拆頁或移至來源附錄**，不允許縮小字體。
...
**留白是設計語言，但結尾不能沒有出口。**
主張句之後，必須有一個行動指向元素（≤ 30 字）：可以是一個核心問題、一個 next step 句、或一個 call to action。字體小（8–9px）、視覺輕（rgba 白色 0.55），不破壞留白節奏，但給讀者閱讀動能的出口。缺少此元素視為頁面未完成。
...
Steal card 的寫作目標不是「完整說明這個方法」，而是「讓讀者知道第一步從哪裡開始」。

**中欄說明文（≤ 70 字）**：說清楚這個方法的核心邏輯和它解決什麼問題。不需要說清楚所有細節——細節在附錄。

**右欄執行（≤ 55 字）**：具體第一步用一個動詞句（≤ 25 字），邊界條件用一句話（≤ 30 字）。前置條件若無法在此範圍內說清楚，移至來源附錄。

**自查標準**：讀者看完這個橫條，能不能說出「我下週第一步要做什麼」？能，就夠了。不能，問題不是字數太少，是邏輯沒說清楚。
